...
na_fit <- survfit(Surv(y, cens) ~ 1, type = "fh")  # Fleming-Harrington estimator (equivalent to NA)
summary(na_fit)

na_fit1 <- survfit(Surv(y, cens) ~ 1, type = "fh",conf.type='plain')  # Fleming-Harrington estimator (equivalent to NA)
summary(na_fit1)
